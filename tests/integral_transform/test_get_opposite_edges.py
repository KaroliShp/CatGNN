...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            torch.tensor([[1, 0, 2, 1, 3, 2], [0, 1, 1, 2, 2, 3]], dtype=torch.int64),
        ),
        # 2 -> 3, but no 3 -> 2
        (
            torch.tensor([[0, 1, 1, 2, 2], [1, 0, 2, 1, 3]], dtype=torch.int64),
            torch.tensor([[1, 0, 2, 1, -1], [0, 1, 1, 2, -1]], dtype=torch.int64),
        ),
        # 2 -> 3 and 3 -> 1, but no 3 -> 2 and 1 -> 3
        (
            torch.tensor([[0, 1, 1, 2, 2, 3], [1, 0, 2, 1, 3, 1]], dtype=torch.int64),
            torch.tensor(
                [[1, 0, 2, 1, -1, -1], [0, 1, 1, 2, -1, -1]], dtype=torch.int64
            ),
        ),
        # Only directed edges (so no pullback from receivers)
        (
            torch.tensor([[0, 1, 2], [1, 2, 3]], dtype=torch.int64),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            torch.tensor([[0, 1, 1, 2, 2], [1, 0, 2, 1, 3]], dtype=torch.int64),
            torch.tensor([[1, 0, 2, 1, 3], [0, 1, 1, 2, 2]], dtype=torch.int64),
        ),
        # 2 -> 3 and 3 -> 1, but no 3 -> 2 and 1 -> 3
        (
            torch.tensor([[0, 1, 1, 2, 2, 3], [1, 0, 2, 1, 3, 1]], dtype=torch.int64),
            torch.tensor([[1, 0, 2, 1, 3, 1], [0, 1, 1, 2, 2, 3]], dtype=torch.int64),
        ),
        # Only directed edges (so no pullback from receivers)
        (
            torch.tensor([[0, 1, 2], [1, 2, 3]], dtype=torch.int64),
            torch.tensor([[1, 2, 3], [0, 1, 2]], dtype=torch.int64),
        ),
    ],
)
