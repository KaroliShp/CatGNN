...
@pytest.mark.parametrize(
    "V,E,X,in_dim,out_dim,expected_X_shape",
    [
        # Undirected graph
        (
...
def test_sage_mpnn_2(V, E, X, in_dim, out_dim, expected_X_shape):
    sage_layer = SAGELayer_MPNN_2(in_dim, out_dim)
    output = sage_layer(V, E, X)

    assert output.shape == expected_X_shape


@pytest.mark.parametrize("in_dim,out_dim", [(2, 2)])
def test_sage_mpnn_2_reset_params(in_dim, out_dim):
    sage_layer = SAGELayer_MPNN_2(in_dim, out_dim)
    before_params = copy.deepcopy([p for p in sage_layer.parameters()])
    sage_layer.reset_parameters()
    after_params = copy.deepcopy([p for p in sage_layer.parameters()])
