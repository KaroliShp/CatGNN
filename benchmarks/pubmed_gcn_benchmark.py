...
def benchmark(model_nn, sender_to_receiver=True):
    """
    Benchmark PubMed dataset
    """
    pubmed_dataset = PlanetoidDataset('PubMed')

    train_x, train_y, val_x, val_y, test_x, test_y = pubmed_dataset.train_val_test_split()
    train_mask, val_mask, test_mask = pubmed_dataset.get_split_masks()

    V = pubmed_dataset.get_vertices()
    E = pubmed_dataset.get_edges(sender_to_receiver)
    X = pubmed_dataset.get_features()

    model = model_nn(input_dim=train_x.shape[-1], output_dim=7)

    train_stats_gnn_pubmed = train_eval_loop(model, 
                                           V, E, X, train_y, train_mask, 
                                           V, E, X, val_y, val_mask, 
                                           V, E, X, test_y, test_mask)
    print(train_stats_gnn_pubmed)
